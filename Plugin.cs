...
public partial class EpicMMOSystem : BaseUnityPlugin
{
    ...
    public void Awake()
    {
        ...
        priceResetPoints = config(levelSystem, "PriceResetPoints", 3,
            "Reset price per point. Цена сброса за один поинт");
        freePointForLevel = config(levelSystem, "FreePointForLevel", 5,
            "Free points per level. Свободных поинтов за один уровень");
        startFreePoint = config(levelSystem, "StartFreePoint", 5,
            "Additional free points start. Дополнительных свободных поинтов");
        levelExp = config(levelSystem, "LevelExperience", 300,
            "Amount of experience needed per level. Количество опыта необходимого на 1 уровень");
        levelexpforeachlevel = config(levelSystem, "Add LevelExperience on each level", true,
            "By default the calculations per level are (previous_amount * 1.05 + 300) disabled it will be (previous_amount * 1.05) per level ");
        multiNextLevel = config(levelSystem, "MultiplyNextLevelExperience", 1.05f,
            "Experience multiplier for the next level - Should never go below 1.00. Умножитель опыта для следующего уровня");
        expForLvlMonster = config(levelSystem, "ExpForLvlMonster", 0.25f,
            "Extra experience (from the sum of the basic experience) for the level of the monster. Доп опыт (из суммы основного опыта) за уровень монстра");
        rateExp = config(levelSystem, "RateExp", 2f, "Experience multiplier. Множитель опыта");
        groupExp = config(levelSystem, "GroupExp", 0.70f,
            "Experience multiplier that the other players in the group get. Множитель опыта который получают остальные игроки в группе");
        ...
        levelsForBinusFreePoint = config(levelSystem, "BonusLevelPoints", "5:5,10:5",
            "Added bonus point for level. Example(level:points): 5:10,15:20 add all 30 points ");
        groupRange = config(levelSystem, "Group EXP Range", 70f,
            "The range at which people in a group (Group MOD ONLY) get XP, relative to player who killed mob - only works if the killer gets xp. - Default 70f, a large number like 999999999999f, will probably cover map");
        ...
        leftMessageXP = config(levelSystem, "Display XP Received on Left", true,
            "Display XP Amount on Left Message when mob is killed");
        ...
        maxValueStrength = config(levelSystem, "maxValueStrength", 200,
            "Maximum number of points you can put into Strength");
        maxValueDexterity = config(levelSystem, "maxValueDexterity", 200,
            "Maximum number of points you can put into Dexterity");
        maxValueIntelligence = config(levelSystem, "maxValueIntelligence", 200,
            "Maximum number of points you can put into Intelleigence");
        maxValueEndurance = config(levelSystem, "maxValueEndurance", 200,
            "Maximum number of points you can put into Endurance");
        maxValueVigour = config(levelSystem, "maxValueVigour", 200, "Maximum number of points you can put into Vigour");
        maxValueSpecializing = config(levelSystem, "maxValueSpecializing", 200,
            "Maximum number of points you can put into Specializing");
        ...
        staminaBlock = config(levelSystemStrngth, "StaminaBlock", 0.2f,
            "Decrease stamina consumption per unit per point.");
        ...
        attackSpeed = config(levelSystemAgility, "AttackSpeed", 0.2f,
            "Swing your blade faster with every point - OP warning-"); // skip bows
        attackStamina = config(levelSystemAgility, "StaminaAttack", 0.1f, "Reduces attack stamina consumption. ");
        staminaReduction = config(levelSystemAgility, "StaminaReduction", 0.15f,
            "Decrease stamina consumption for running, jumping for one point.");
        ...
        magicEitrRegen = config(levelSystemIntellect, "MagicEitrReg", 0.3f,
            "Increase magical Eitr Regeneration per point.");
        addEitr = config(levelSystemIntellect, "AddEitr", 0.3f,
            "Eitr Increase per point ONLY when player has above 1 base Eitr");
        ...
        critChance = config(levelSystemSpecializing, "Critical Chance", 0.1f,
            "Critical Chance, Starts with CriticalStartChance amount(0%), increment per point - default 0.1% ");
        miningSpeed = config(levelSystemSpecializing, "MiningSpeed", 0.4f, "Mining Dmg Multiplier per point"); //check
        constructionPieceHealth = config(levelSystemSpecializing, "PieceHealth", 2f,
            "Increase max health of new pieces built per point"); // check
        treeCuttingSpeed = config(levelSystemSpecializing, "TreeCuttingSpeed", 0.4f,
            "Increase tree cutting speed per point.");

        string levelSystemHardcore = "1.LevelSystem Hardcore-----------";
        counterCheckDistance = config(levelSystemHardcore, "counterCheckDistance", 50f,
            "Радиус поиска людей со счетчиками убийств босса");
        bossLevelCounterMultiplier = config(levelSystemHardcore, "bossLevelCounterMultiplier", 3,
            "Увеличение уровня босса за каждое убийство");
        ...
        enabledLevelControl = config(creatureLevelControl, "Enabled_creature_level", true,
            "Enable creature Level control - Disable this to remove levels, but still get eXP gain");
        removeDropMax = config(creatureLevelControl, "Remove_creature_drop_max", false,
            "Monsters after death do not give items if their level is higher than player level + MaxLevel");
        removeDropMin = config(creatureLevelControl, "Remove_creature_drop_min", false,
            "Monsters after death do not give items if their level is lower than player level - MinLevel");
        removeBossDropMax = config(creatureLevelControl, "Remove_boss_drop_max", false,
            "Bosses after death do not give items if their level is higher than player level + MaxLevel");
        removeBossDropMin = config(creatureLevelControl, "Remove_boss_drop_min", false,
            "Bosses after death do not give items if their level is lower than player level - Minlevel");
        curveExp = config(creatureLevelControl, "Curve_creature_exp", true,
            "Monsters after death will give less exp if player is outside Max or Min Level Range");
        curveBossExp = config(creatureLevelControl, "Curve_Boss_exp", true,
            "Bosses after death will give less exp if player is outside Max or Min Level Range");
        lowDamageLevel = config(creatureLevelControl, "Low_damage_level", false,
            "Decreased damage to the monster if the level is insufficient");
        lowDamageExtraConfig = config(creatureLevelControl, "Low_damage_config", 0,
            "Extra paramater to low damage config - for reference(float)(playerLevel + lowDamageConfig) / monsterLevel; when player is below lvl");
        minLevelExp = config(creatureLevelControl, "MinLevelRange", 10,
            "Character level - MinLevelRange is less than the level of the monster, then you will receive reduced experience. Уровень персонажа - MinLevelRange меньше уровня монстра, то вы будете получать урезанный опыт");
        maxLevelExp = config(creatureLevelControl, "MaxLevelRange", 10,
            "Character level + MaxLevelRange is less than the level of the monster, then you will not receive experience. Уровень персонажа + MaxLevelRange меньше уровня монстра, то вы не будете получать опыт");
        MobLevelPosition = config(creatureLevelControl, "LevelBar Position", new Vector2(40, -30),
            "LevelBar Position for regular mobs - synced");
        BossLevelPosition = config(creatureLevelControl, "Boss LevelBar Position", new Vector2(0, 30),
            "LevelBar Position for Boss Bars - synced");
        MobLVLChars = config(creatureLevelControl, "Mob Level UI String", "[@]",
            "[@] uses @ for moblevel, must include '@', but you coudl do 'Level @' or something similar");
        removeAllDropsFromNonPlayerKills = config(creatureLevelControl, "RemoveAllDrops From NonPlayer Kills", true,
            "Remove all drops from mobs that were not killed by a player or tame -ie no drops from mobs attacking each other");
        noExpPastLVL = config(creatureLevelControl, "U Jerk, NoExpOn Red/Blue", false,
            "You are a jerk admin if you enable this, this removes all exp on creatures that are above level (RED) or Below (Blue)");
        ...
        string hud = "4.Hud--------------------";
        oldExpBar = config(hud, "OldXPBar", false,
            "Use the old eXP Bar only (need restart, not server sync) Does not move or scale", false);
        showMaxHp = config(hud, "ShowMaxHp", true, "Show max hp (100 / 100)", false);

        HudPanelPosition = config(hud, "1.0HudGrouplPosition", new Vector2(0, 0),
            "Position of the Main EpicHudBarBackground in (x,y) - You can drag individual components below", false);
        HudExpBackgroundCol = config(hud, "1.1BackgroundCol", "#2F1600",
            "Background color in Hex, set to 'none' to make transparent", false);
        HudBarScale = config(hud, "1.2HudGroupScale", .90f,
            "Scale for HudGroup - exp, background, hp, stamina, eitr - You can do individual below", false);
        ExpPanelPosition = config(hud, "2.0ExpPanelPosition", new Vector2(0, 0), "Position of the Exp panel (x,y)",
            false);
        ExpColor = config(hud, "2.1ExpColor", "#FFFFFF",
            "Exp fill color in Hex - White bleeds through with purple, set to 'none' to have no xp bar", false);
        ExpScale = config(hud, "2.2ExpScale", new Vector3(1, 1, 1), "Exp Bar Scale factor", false);
        StaminaPanelPosition = config(hud, "3.0StaminaPanelPosition", new Vector2(0, 0),
            "Position of the Stamina panel (x,y)", false);
        StaminaColor = config(hud, "3.1StaminaColor", "#986100", "Stamina color in Hex, set to 'none' to make vanilla",
            false);
        ...
        EitrPanelPosition = config(hud, "5.0EitrPanelPosition", new Vector2(0, 0), "Position of the Eitr panel (x,y)",
            false);
        ...
        LevelHudGroupScale = config(hud, "6.2LevelHudGroupScale", 1.0f,
            "LevelHud Group of Objects Scale factor (Nav Bar, Point Hub)", false);
        LevelPointPosition = config(hud, "7.0PointHudPosition", new Vector2(0, 0), "Position of the Point panel (x,y)",
            false);
        ...
        OrbSEtime = config(OrbandPotion, "Orb SE time", 10,
            "Orb Consumption SE Icon lasts for default 10 seconds - dont set to 0");
        PotionSEtime = config(OrbandPotion, "Potion SE time", 600, "Potion SE timer last for default of 10 minutes");
        OrbDropChance = config(OrbandPotion, "Orb Drop Chance", 1f,
            "Chance for Magic Orb to drop from a monster - default 1%");
        OrbDropChancefromBoss = config(OrbandPotion, "Ord Drop Boss", 100f,
            "Drop Chance for Orbs to drop from a boss - default 100%");
        OrdDropMaxAmountFromBoss = config(OrbandPotion, "Orb Boss Max Amount", 3,
            "Max Amount of Orbs to drop from Boss if any orbs drop. So there is a chance 1-3 will drop on default");
        ...
        Logger.LogInfo("Config Done");
        
        Localizer.AddPlaceholder("mmoxpdrink1_description", "power1", XPforMinorPotion,
            power1 => ((power1 - 1) * 100).ToString());
        Localizer.AddPlaceholder("mmoxpdrink2_description", "power2", XPforMediumPotion,
            power2 => ((power2 - 1) * 100).ToString());
        Localizer.AddPlaceholder("mmoxpdrink3_description", "power3", XPforGreaterPotion,
            power3 => ((power3 - 1) * 100).ToString());
    ...
    private void ReadJsonValues(object sender, FileSystemEventArgs e)
    {
        if (NetisActive)
        {
            if (ZNet.instance.IsServer() && ZNet.instance.IsDedicated()) // only dedicated
            {
                ...
                {
                    if (peer == null) return;
                    ZRoutedRpc.instance.InvokeRoutedRPC(peer.m_uid, $"{EpicMMOSystem.ModName} SetMonsterDB",
                        list); //sync list
                }
            }
        }
